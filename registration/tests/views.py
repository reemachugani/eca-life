...
class RegisterTests(test.TestCase):
    ...
    def test_renders_to_correct_template(self):
        response = self.client.get(reverse("registration_register"))
        self.assertTemplateUsed(response, "registration_old/register.html")
    ...
    @patch("registration_old.views.get_site")
    @patch("registration_old.models.RegistrationProfile.objects")
    def test_creates_inactive_user_on_success(self, manager, get_site):
        ...
        self.assertEqual(((data['username'], data['password1'], data['email'],
                           get_site.return_value), {}),
...
class RegistrationCompleteTests(test.TestCase):
    ...
    def test_renders_to_correct_template(self):
        response = self.client.get(reverse("registration_complete"))
        self.assertTemplateUsed(response, "registration_old/registration_complete.html")

class ActivateTests(test.TestCase):

    def test_accesses_page_successfully(self):
        response = self.client.get(reverse("registration_activate",
                                           kwargs={'activation_key': '123'}))
        self.assertEqual(200, response.status_code)

    def test_shows_activation_failed_template_when_key_not_valid(self):
        response = self.client.get(reverse("registration_activate",
                                           kwargs={'activation_key': '123'}))
        self.assertTemplateUsed(response, "registration_old/activation_failed.html")

    @patch('registration_old.models.RegistrationProfile.objects.activate_user',
